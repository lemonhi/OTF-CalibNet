...
class BaseKITTIDataset(Dataset):
    def __init__(self, basedir: str, batch_size: int, seqs=['04', '05'], cam_id: int = 2,
        ...
        self.kitti_datalist = [pykitti.odometry(basedir, seq, frames=frame) for seq, frame in zip(seqs, frame_list) if
                               len(frame) > 0]
    ...
    def check(odom_obj: pykitti.odometry, cam_id: int, seq: str) -> bool:
        ...
        assert cam_files_length == velo_files_lenght, head_msg + "number of cam %d (%d) and velo files (%d) doesn't equal!" % (
        cam_id, cam_files_length, velo_files_lenght)
...
class KITTI_perturb(Dataset):
    def __init__(self, dataset: BaseKITTIDataset, max_deg: float, max_tran: float, mag_randomly=True, pooling_size=5,
        ...
        if self.file is not None:
            self.perturb = torch.from_numpy(np.loadtxt(self.file, dtype=np.float32, delimiter=','))[
                None, ...]  # (1,N,6)
        else:
...
if __name__ == "__main__":
    base_dataset = BaseKITTIDataset('../data', 1, seqs=['00'], skip_frame=3)
    dataset = KITTI_perturb(base_dataset, 10, 0.2, file='./checkpoint/test_seq_10_0.2.csv')
    print(len(dataset))
    ...
    for i in range(len(dataset)):
        ...
        for key, value in data.items():
            ...
            # 分别保存depth_img和uncalibed_depth_img
            if key == 'depth_img':
                plt.figure()
                plt.imshow(value.squeeze(0))
                plt.savefig(os.path.join(depth_img_dir, 'depth_img_{}.png'.format(i)))
                plt.close()
            if key == 'uncalibed_depth_img':
                plt.figure()
                plt.imshow(value.squeeze(0))
                plt.savefig(os.path.join(uncalib_depth_img_dir, 'uncalib_depth_img_{}.png'.format(i)))
                plt.close()